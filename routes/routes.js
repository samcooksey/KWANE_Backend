...
router.post('/newGame', function(req, res, next){
  ...
  console.log(chosenPuzzles);
  var orientation = letters[Math.floor(Math.random() * 4)];
  knex('puzzles')
  .then(function(data){
    // console.log({player_name: req.body.partnerName, oracle_name: req.body.oracleName, puzzle_1: data[chosenPuzzles[0]].puzzle_array, puzzle_1_payload: shuffledPayloads[0], puzzle_1_orientation: (data[chosenPuzzles[0]].id % 2 === 0), id1: data[chosenPuzzles[0]].id, puzzle_2: data[chosenPuzzles[1]].puzzle_array, puzzle_2_payload: shuffledPayloads[1], puzzle_2_orientation: (data[chosenPuzzles[1]].id % 2 === 0), id2: data[chosenPuzzles[1]].id, puzzle_3: data[chosenPuzzles[2]].puzzle_array, puzzle_3_payload: shuffledPayloads[2], puzzle_3_orientation: (data[chosenPuzzles[2]].id % 2 === 0), id3: data[chosenPuzzles[2]].id, puzzle_4: data[chosenPuzzles[3]].puzzle_array, puzzle_4_payload: shuffledPayloads[3], puzzle_4_orientation: (data[chosenPuzzles[3]].id % 2 === 0), id4: data[chosenPuzzles[3]].id, orientation: orientation });
    return knex('games')
